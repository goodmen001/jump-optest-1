...
# 修改主机名以及一些显示信息
sed -i "s/hostname='*.*'/hostname='hackroom'/" package/base-files/files/bin/config_generate
...
sed -i '/(<%=pcdata(ver.luciversion)%>)/a\      built by 仅限测试学习，24小时后必须删除，否则后果自负' package/lean/autocore/files/x86/index.htm
