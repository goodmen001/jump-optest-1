...
# 修改主机名以及一些显示信息
sed -i "s/hostname='*.*'/hostname='FSX'/" package/base-files/files/bin/config_generate
...
sed -i '/(<%=pcdata(ver.luciversion)%>)/a\      built by 学习用不可用于其它用途' package/lean/autocore/files/x86/index.htm
...
sed -i 's/ssid=*.*/ssid=FSX/g' package/kernel/mac80211/files/lib/wifi/mac80211.sh
...
#git clone -b master https://github.com/pymumu/openwrt-smartdns package/lean/smartdns #GG
#git clone -b lede https://github.com/pymumu/luci-app-smartdns package/lean/luci-app-smartdns#GG
